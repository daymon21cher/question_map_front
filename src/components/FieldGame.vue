...
export default {
  ...
  methods: {
    ...
    handleLogin() {
      ...
      console.log(data);
      axios.post('http://tabquest.ru/api/field/login/', data, {
    ...
    loadCells() {
      axios.get('http://tabquest.ru/api/field/custom_action/', {
        withCredentials: true,
      })
          .then(res => {
    ...
    handleAnswerSubmit() {
      ...
      axios.post('http://tabquest.ru/api/cell/' + this.clickedCellId + '/take_answer/', data, {
        withCredentials: true,
      })
          .then(res => {
